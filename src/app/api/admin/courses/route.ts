...
async function ensureUniqueSlug(tenantId: string, baseSlug: string) {
  let slug = baseSlug;
  let i = 2;
  while (true) {
...
export async function POST(req: Request) {
  ...
  const slug = await ensureUniqueSlug(tenantId, baseSlug || `course-${Date.now()}`);

  const publishedAt =
    parsed.data.status === "PUBLISHED" || parsed.data.status === "FEATURED" ? new Date() : null;
